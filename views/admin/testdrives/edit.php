...
<div class="card shadow-sm" id="formView">
    ...
    <div class="card-body">
        <form method="post" action="/admin/testdrives/update" class="">
            <input type="hidden" name="id" value="<?= $this->e(is_object($testdrive) ? $testdrive->id : ($testdrive['id'] ?? '')) ?>">
            <div class="row">
                <div class="col-md-6 mb-3">
                    <label for="id_user" class="form-label">Usuário</label>
                    <select class="form-control" id="id_user" name="id_user" required>
                        <option value="">Selecione o Usuário</option>
                        <?php $current_user_id = is_object($testdrive) ? ($testdrive->id_user ?? '') : ($testdrive['id_user'] ?? ''); ?>
...
```
            <div class="col-md-6 mb-3">
                ...
                <select class="form-control" id="id_carro" name="id_carro" required>
                    ...
                    <?php $current_carro_id = is_object($testdrive) ? ($testdrive->id_carro ?? '') : ($testdrive['id_carro'] ?? ''); ?>
            ...
            <div class="col-md-6 mb-3">
                ...
                <input type="date" class="form-control" id="data_testdrive" name="data_testdrive"
                       value="<?= $this->e(is_object($testdrive) ? ($testdrive->data_testdrive ?? date('Y-m-d')) : ($testdrive['data_testdrive'] ?? date('Y-m-d'))) ?>" required>
            ...
            <div class="col-md-6 mb-3">
                ...
                <input type="date" class="form-control" id="data_devolucao" name="data_devolucao"
                       value="<?= $this->e(is_object($testdrive) ? ($testdrive->data_devolucao ?? '') : ($testdrive['data_devolucao'] ?? '')) ?>">
            ...
            <div class="col-md-6 mb-3">
                ...
                <select class="form-control" id="status" name="status" required>
                    <?php $current_status = is_object($testdrive) ? ($testdrive->status ?? 'pendente') : ($testdrive['status'] ?? 'pendente'); ?>
